...
5. Controller unlocks `Ftst`, enters manual mode, writes max targets, and polls
   actual RPM.
...
3. Controller writes protective high targets, releases manual mode, turns
   `Ftst` off, restores captured targets, and waits for managed mode.
...
- No lowercase mode-key path until validated.
...
.build/release/coldfront read FNum F0Ac F0Tg F0Md F1Ac F1Tg F1Md Ftst RPlt
```
